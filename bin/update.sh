#!/usr/bin/zsh
echo "This script should only be run if you need to mass update all components; do so by commenting out this line and running."
exit 0
...
if [ -d "$WORKDIR/.subsplit/.git/subtree-cache" ];then
    echo "Previous run does not appear to have completed!" ;
    exit 2 ;
fi
